...
// Chat interno estilo WhatsApp entre gestor e empresas
let managerConversations = [];
let selectedConversationId = null;
let selectedConversation = null;
let managerMessagesUnsub = null;
function startManagerChat(){
  onSnapshot(query(collection(db,"conversas"), orderBy("atualizadoEm","desc"), limit(80)), snap=>{
    managerConversations = snap.docs.map(d=>({id:d.id,...d.data()}));
    renderManagerChatList();
  }, console.warn);
  $("chatCompanySearch")?.addEventListener("input", renderManagerChatList);
}
function ensureCompanyConversations(){
  companies.slice(0,80).forEach(c=>{
    setDoc(doc(db,"conversas",c.id), { empresaId:c.id, empresaNome:c.empresaNome||c.nome||"Empresa", atualizadoEm:serverTimestamp(), ultimaMensagem:"Canal aberto" }, {merge:true}).catch(()=>{});
  });
}
function renderManagerChatList(){
  const el=$("managerChatList"); if(!el) return;
  const q=($("chatCompanySearch")?.value||"").toLowerCase();
  const byConv = new Map(managerConversations.map(c=>[c.id,c]));
  const merged = [...managerConversations];
  companies.forEach(c=>{ if(!byConv.has(c.id)) merged.push({id:c.id, empresaId:c.id, empresaNome:c.empresaNome||c.nome||"Empresa", ultimaMensagem:"Canal aberto", naoLidasGestor:0}); });
  const list = merged.filter(c=>`${c.empresaNome} ${c.ultimaMensagem||""}`.toLowerCase().includes(q)).slice(0,80);
  el.innerHTML = list.map(c=>`<article class="chat-contact ${selectedConversationId===c.id?"active":""}" onclick="abrirConversaGestor('${c.id}')"><strong>${safeText(c.empresaNome||"Empresa")}</strong><span>${safeText(c.ultimaMensagem||"Canal aberto")}</span>${Number(c.naoLidasGestor||0)>0?`<small>${Number(c.naoLidasGestor)} nova(s)</small>`:""}</article>`).join("") || `<div class="empty">Nenhuma empresa encontrada.</div>`;
}
window.abrirConversaGestor = async (id)=>{
  selectedConversationId = id;
  selectedConversation = managerConversations.find(c=>c.id===id) || companies.find(c=>c.id===id) || {id, empresaNome:"Empresa"};
  await setDoc(doc(db,"conversas",id), { empresaId:id, empresaNome:selectedConversation.empresaNome||selectedConversation.nome||"Empresa", atualizadoEm:serverTimestamp() }, {merge:true});
  const header=$("managerChatHeader");
  if(header) header.innerHTML = `<div><strong>${safeText(selectedConversation.empresaNome||selectedConversation.nome||"Empresa")}</strong><span>Conversa operacional</span></div><a class="button-link" target="_blank" href="${whatsappUrl(selectedConversation.telefone||MANAGER_PHONE, `Olá, aqui é o gestor FreteHub.`)}">WhatsApp</a>`;
  if($("managerChatInput")) $("managerChatInput").disabled=false;
  renderManagerChatList();
  managerMessagesUnsub?.();
  managerMessagesUnsub = onSnapshot(query(collection(db,"conversas",id,"mensagens"), orderBy("criadoEm","asc"), limit(80)), snap=>{
    const msgs=snap.docs.map(d=>({id:d.id,...d.data()}));
    const el=$("managerChatMessages"); if(!el) return;
    el.innerHTML = msgs.map(m=>`<div class="bubble ${m.autorTipo==="gestor"?"me":"other"}"><strong>${m.autorTipo==="gestor"?"Gestor FreteHub":safeText(selectedConversation.empresaNome||selectedConversation.nome||"Empresa")}</strong>${safeText(m.texto)}<small>${formatDate(m.criadoEm)}</small></div>`).join("") || `<div class="empty">Envie a primeira mensagem para esta empresa.</div>`;
    el.scrollTop=el.scrollHeight;
    updateDoc(doc(db,"conversas",id),{naoLidasGestor:0}).catch(()=>{});
  }, console.warn);
};
$("managerChatForm")?.addEventListener("submit", async e=>{
  e.preventDefault();
  if(!selectedConversationId) return;
  const input=$("managerChatInput"); const text=(input?.value||"").trim(); if(!text) return;
  await addDoc(collection(db,"conversas",selectedConversationId,"mensagens"), { autorId:currentUser?.uid||"gestor", autorTipo:"gestor", texto:text, criadoEm:serverTimestamp(), lida:false });
  await setDoc(doc(db,"conversas",selectedConversationId), { empresaId:selectedConversationId, empresaNome:selectedConversation?.empresaNome||selectedConversation?.nome||"Empresa", ultimaMensagem:text, atualizadoEm:serverTimestamp(), naoLidasEmpresa:1 }, {merge:true});
  input.value="";
});
setTimeout(()=>{ if(currentUser){ startManagerChat(); ensureCompanyConversations(); } }, 1200);
const originalFillCompanySelect = fillCompanySelect;
fillCompanySelect = function(){ originalFillCompanySelect(); ensureCompanyConversations(); };
